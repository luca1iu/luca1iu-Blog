 ...
 * @returns {*} 处理后的链接
 */
export function convertMultiLangUrl(str, currentPath = '') {
  if (!str) {
    return '#'
  }
  ...
  // 如果当前路径包含语言前缀，且目标链接也包含语言前缀
  // 则需要确保跳转到正确的语言页面
  const langPrefixes = ['zh', 'de', 'en', 'fr', 'ja', 'tr']
  const currentLang = langPrefixes.find(lang => currentPath.startsWith(`/${lang}`))
  const targetLang = langPrefixes.find(lang => str.startsWith(`/${lang}`))
  
  // 如果当前在某个语言页面，但目标链接指向另一个语言页面
  // 则直接返回目标链接，让用户跳转到正确的语言页面
