...
export async function getGlobalData({
  ...
  // 调试信息
  console.log('[getGlobalData] locale:', locale)
  console.log('[getGlobalData] siteIds:', siteIds)
  console.log('[getGlobalData] from:', from)
  ...
  try {
    ...
    for (let index = 0; index < siteIds.length; index++) {
      ...
      console.log('[getGlobalData] checking siteId:', siteId, 'prefix:', prefix, 'locale:', locale)
      
      // 如果指定了语言，且匹配当前数据库的语言前缀
      // 支持多种匹配方式：de, de-DE, de_DE 等
      if (locale && prefix) {
        const localeLang = locale.split(/[-_]/)[0] // 提取语言部分，如 'de' from 'de-DE'
        const prefixLang = prefix.split(/[-_]/)[0] // 提取前缀语言部分
        
              if (localeLang === prefixLang || locale === prefix) {
        console.log('[getGlobalData] Found matching language database:', prefix, 'for locale:', locale)
        matchedData = await getSiteDataByPageId({
          pageId: id,
          from,
          locale
        })
    ...
    // 如果找到了匹配的语言，使用它；否则尝试其他数据库
    if (matchedData) {
      console.log('[getGlobalData] Using matched language data')
      data = matchedData
    } else {
      console.log('[getGlobalData] No matching language found, trying other databases...')
      ...
      for (let index = 0; index < siteIds.length; index++) {
        ...
        console.log('[getGlobalData] Trying database:', siteId, 'prefix:', prefix)
        
        try {
          const testData = await getSiteDataByPageId({
            pageId: id,
            from: from + '-fallback',
            locale
          })
          ...
          // 检查是否有实际内容（不是错误页面）
          if (testData?.allPages?.length > 0 && 
              testData.allPages[0]?.title && 
              !testData.allPages[0]?.title.includes('无法获取Notion数据')) {
            console.log('[getGlobalData] Found database with content:', prefix || 'default')
            data = testData
            break
          }
        } catch (error) {
          console.log('[getGlobalData] Failed to get data from:', siteId, error.message)
        }
      }
      
      // 如果所有数据库都没有内容，使用第一个
      if (!data) {
        console.log('[getGlobalData] No database has content, using first one')
        const firstSiteId = siteIds[0]
        const firstId = extractLangId(firstSiteId)
 ...
 */
export async function getSiteDataByPageId({ pageId, from, locale }) {
  // 获取NOTION原始数据，此接支持mem缓存。
  // 为不同语言使用不同的缓存键
  const cacheKey = locale ? `site_data_${pageId}_${locale}` : `site_data_${pageId}`
  
  console.log('[getSiteDataByPageId] Using cache key:', cacheKey, 'for pageId:', pageId, 'locale:', locale)
