...
export function generateLocaleDict(langString) {
  ...
  console.log('[generateLocaleDict] Processing language:', langString, 'language:', language, 'region:', region)

  // 优先匹配语言和地区都匹配的情况
  const specificLocale = `${language}-${region}`
  if (supportedLocales.includes(specificLocale)) {
    userLocale = LANGS[specificLocale]
    console.log('[generateLocaleDict] Found specific locale:', specificLocale)
  }
  ...
  if (!userLocale) {
    ...
    if (languageOnlyLocales.length > 0) {
      ...
      console.log('[generateLocaleDict] Found language-only locale:', languageOnlyLocales[0])
    }
  }
  ...
  if (!userLocale) {
    ...
    console.log('[generateLocaleDict] Using fallback locale:', fallbackLocale)
  }
...
export function initLocale(locale, changeLang, updateLocale) {
  if (isBrowser) {
    ...
    if (queryLang) {
      // 支持更灵活的语言匹配，包括 de, de-DE, de_DE 等格式
      const match = queryLang.match(/[a-zA-Z]{2}(?:[-_][a-zA-Z]{2})?/)
      if (match) {
        const targetLang = match[0]
        console.log('[initLocale] Setting language to:', targetLang)
...
export const redirectUserLang = (lang, pageId) => {
  ...
  console.log('[redirectUserLang] userLang:', userLang, 'siteIds:', siteIds)

  // 默认是进首页; 如果检测到有一个多语言匹配了用户浏览器，则自动跳转过去
  for (let index = 0; index < siteIds.length; index++) {
    const siteId = siteIds[index]
    const prefix = extractLangPrefix(siteId)
    console.log('[redirectUserLang] checking siteId:', siteId, 'prefix:', prefix)
    
    // 支持更灵活的语言匹配
    if (prefix && userLang) {
      const userLangCode = userLang.split(/[-_]/)[0] // 提取语言代码
      const prefixCode = prefix.split(/[-_]/)[0] // 提取前缀语言代码
      
      if (prefixCode === userLangCode || prefix === userLang) {
        if (window.location.pathname.indexOf(prefix) < 0) {
          console.log('[redirectUserLang] Redirecting to:', '/' + prefix)
